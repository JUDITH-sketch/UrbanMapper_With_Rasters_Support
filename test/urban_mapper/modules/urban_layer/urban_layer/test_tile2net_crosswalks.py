...
from urban_mapper.modules import Tile2NetCrosswalks
import pytest

@pytest.mark.skip()
class TestTile2NetCrosswalks:
  """
    It tests a Tile2NetCrosswalks class.

  """
  layer = Tile2NetCrosswalks()
  loader = um.UrbanMapper().loader

  crosswalk_path = "test/data_files/small_NYC-Polygons-09-07-2025_16_09/NYC-Polygons-09-07-2025_16_09.shp"
  ...
  def test_from_file(self):
    assert self.layer.from_file(self.crosswalk_path) is None
  ...
  def test_map_nearest_layer(self):
    ...
    assert self.layer.map_nearest_layer(
      ...
      output_column="crosswalk_near"
    ) is not None

    """
        Geometry columns
    """
    self.layer = Tile2NetCrosswalks() # It is not possible to map the same layer twice
    self.layer.from_file(self.crosswalk_path)
