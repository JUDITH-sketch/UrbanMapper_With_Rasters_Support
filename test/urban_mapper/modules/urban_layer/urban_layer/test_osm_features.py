...
class TestOSMFeatures:
  ...
  def test_from_place(self):
    assert self.layer.from_place("Downtown, Brooklyn, New York, USA", tags={"leisure": ["park", "garden"]}) is None

  def test_from_address(self):
    assert self.layer.from_address("370 Jay St, Brooklyn, New York 11201, United States", tags={"building": True}, dist=50.0) is None
  ...
  def test_from_bbox(self):
    ...
    assert self.layer.from_bbox((-74.01, 40.70, -73.97, 40.72), tags={"leisure": ["park", "garden"]}) is None

  def test_from_point(self):
    # Bryan Park
    # (lat, lon) center point
    assert self.layer.from_point((40.7536, -73.9832), tags={"building": True}, dist=50.0) is None
  ...
  def test_from_polygon(self):
    ...
    assert self.layer.from_polygon(polygon, tags={"leisure": ["park", "garden"]}) is None
  ...
  def test_map_nearest_layer(self):
    """
        Lat/Long columns
    """
    self.layer.from_place("Downtown, Brooklyn, New York, USA", tags={"leisure": ["park", "garden"]})
