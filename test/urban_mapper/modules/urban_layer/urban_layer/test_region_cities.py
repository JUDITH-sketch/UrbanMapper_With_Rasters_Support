...
from urban_mapper.modules import RegionCities
import pytest

@pytest.mark.skip()
class TestRegionCities:
  """
  ...
  def test_from_place(self):
    assert self.layer.from_place("New York, New York, USA") is None
  ...
  def test_map_nearest_layer(self):
    ...
    """
        Geometry columns
    """
    self.layer = RegionCities() # It is not possible to map the same layer twice
    self.layer.from_place("New York, New York, USA")
