...
from urban_mapper.modules import OSMNXStreets
import pytest

@pytest.mark.skip()
class TestOSMNXStreets:
  """
    It tests a OSMNXStreets class.
  ...
  def test_from_address(self):
    assert self.layer.from_address("370 Jay St, Brooklyn, New York 11201, United States", dist=50.0) is None
  ...
  def test_from_point(self):
    ...
    assert self.layer.from_point((40.7536, -73.9832), dist=100.0) is None
  ...
  def test_from_xml(self):
    assert self.layer.from_xml(filepath = "test/data_files/bryant_park.osm") is None
  ...
  def test_map_nearest_layer(self):
    ...
    """
        Geometry columns
    """
    self.layer = OSMNXStreets() # It is not possible to map the same layer twice
    self.layer.from_place("Downtown, Brooklyn, New York, USA")
