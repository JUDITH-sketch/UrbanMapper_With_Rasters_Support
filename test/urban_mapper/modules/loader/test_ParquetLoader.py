...
class TestParquetLoader:
  ...
  def test_load_data_from_file(self):
    """
        Lat/Long columns
    """
    loader = ParquetLoader(self.file_path, longitude_column="longitude", latitude_column="latitude")
    ...
    """
    loader = ParquetLoader(self.file_path, geometry_column="the_geom", coordinate_reference_system="EPSG:4326")
    assert isinstance(loader.load_data_from_file(), gpd.GeoDataFrame)

    """
        Source-target coordinate references
    """
    loader = ParquetLoader(self.file_path, geometry_column="the_geom", coordinate_reference_system=("EPSG:4326", "EPSG:3857"))
    assert isinstance(loader.load_data_from_file(), gpd.GeoDataFrame)

    """
        Map column names
    """
    loader = ParquetLoader(self.file_path, geometry_column="the_geom", map_columns={"the_geom": "geometry"})
    assert isinstance(loader.load_data_from_file(), gpd.GeoDataFrame)    
