...
class TestAddressGeoImputer:
  ...
  def test_transform(self):
    """
        Lat/Long columns
    """    
    imputer = AddressGeoImputer(latitude_column="latitude", longitude_column="longitude", address_column="fake_address")
    assert imputer.transform(self.data_neigborhood_latlong, self.layer) is not None

    """
        Geometry columns
    """
    imputer = AddressGeoImputer(geometry_column="geometry", address_column="fake_address")
    ...
    """
    data_neigborhood = {"neighborhood": self.data_neigborhood_geom, "speed_humps": self.data_speed_hump}

    imputer = AddressGeoImputer(geometry_column="geometry", address_column="fake_address", data_id="neighborhood")
    assert imputer.transform(data_neigborhood, self.layer) is not None    
        
  def test_preview(self):
    loader = AddressGeoImputer(longitude_column="longitude", latitude_column="latitude")
